 ...
 */
	public function add() {
		...
		$vendorCats = $this->Vendor->VendorCat->find('list');
		$ratings = $this->Vendor->Rating->find('list');
		$prices = $this->Vendor->Price->find('list');
		$cities = $this->Vendor->City->find('list');
		$states = $this->Vendor->State->find('list');
		$this->set(compact('vendorCats', 'ratings', 'prices', 'cities', 'states'));
	}
