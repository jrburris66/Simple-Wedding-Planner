...
<head>
	...
	<?php
		...
		echo $this->Html->css('scrm.generic');
		echo $this->Html->css('jb.generic');
...
<body>
	<div id="container">
		<div id="header">
			<h1><?php echo (isset($Tr['SiteTitle'])) ? $this->Html->link(__($Tr['SiteTitle']), '/') : $this->Html->link(__('Home'), '/'); ?></h1>
				...
				| <?php echo $this->Html->link(__("Logout",true),"/logout") ?></div>
                        <?php } else { ?>
                                <div id="jb_header_auth"><?php echo $this->Html->link(__("Login",true),"/") ?></div>
                        <?php } ?>
		</div>
		<div id="content">
		...
		<div id="footer">
			...
			Powered by <a href="#">Southern CRM</a> | 
